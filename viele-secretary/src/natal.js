...
/* ════════════════════════════════════════════════════════════════
   守護神（調候用神）── AI不使用・完全決定論 ──

   ◆ 概念
     一般に占いで言う「守護神」は四柱推命の「調候用神（ちょうこうようじん）」を指す。
     命式の寒暖・燥湿のバランスを整える十干で、「日干 × 生まれ月（節月＝月支）」で決まる。
     季節（月支）によって命式は寒い/暑い/乾く/湿るに偏るため、それを中和する五行
     （＝調候用神）を補うと運が開く、という考え方。

   ◆ 出典・採用した表
     『窮通宝鑑（きゅうつうほうかん）／欄江網（らんこうもう）』系の調候用神表に基づく。
     窮通宝鑑は調候用神を体系化した古典で、徐樂吾の評註本などで広く流布する。
     本実装は「各 日干 × 月支 の “主たる調候用神（第一用神）” を1つ」確定させた
     120マス表（下記 CHOKO 表）。複数の用神を要する命式も多いが、ここでは
     最も代表的な第一用神（主）を採り、必要に応じ副用神(sub)を併記する。

   ◆ 流派差について（正直な明示）
     窮通宝鑑の用神は版・評註者により細部が異なり（例：丙火・庚金の用神順、
     土性日干の扱い等）、また実際の鑑定では命式全体の強弱で用神を調整する。
     本表は「窮通宝鑑系で広く採られる代表的な第一用神」を決定論的に固定したもので、
     個別命式の精密な用神とは差異が出ることがある（簡略化の明示）。

   ◆ 表の見方（CHOKO[日干][月支] = [主用神, 副用神?]）
     月支は節月（立春起点）。寅=1月節〜丑=12月節。
     値は調候用神の十干。副用神は補助的に効く干（無い場合は省略）。
   ════════════════════════════════════════════════════════════════ */

// 五行 → 開運色・方位（一般的な五行配当）。木=緑/東, 火=赤/南, 土=黄/中央, 金=白/西, 水=黒青/北。
const ELEM_COLOR = { 木: "緑", 火: "赤", 土: "黄", 金: "白", 水: "黒・青" };
const ELEM_DIR = { 木: "東", 火: "南", 土: "中央", 金: "西", 水: "北" };

// 十干 → 読み（label 用）。例「壬水」。
const STEM_ELEM_LABEL = (stem) => `${stem}${FIVE[stem]}`;

// 守護神（調候用神）の意味・活かし方（主用神の十干ごと）。1〜2文。
const SHUGO_MEANING = {
  甲: "甲（大樹の木）が守護神。背筋を伸ばして真っ直ぐ進み、長期で育てるテーマを一本柱に据えると運が開きます。",
  乙: "乙（草花の木）が守護神。しなやかに人や環境へ適応し、ご縁を絡めて広げると運が伸びます。",
  丙: "丙（太陽の火）が守護神。明るく前向きに表へ出て、人を照らす発信・露出を増やすと運が開きます。",
  丁: "丁（灯火の火）が守護神。一点に集中して灯すように、専門性と細やかな心配りを磨くと運が育ちます。",
  戊: "戊（山の土）が守護神。どっしり構えて土台を固め、焦らず安定を作ることで運が安定します。",
  己: "己（田畑の土）が守護神。地道に育て・運用し、実りを積み重ねる姿勢が運を呼びます。",
  庚: "庚（鉄の金）が守護神。思い切った決断と実行で不要を断ち切ると、道が開けます。",
  辛: "辛（宝石の金）が守護神。質と完成度にこだわり、自分を磨いて価値を高めると運が上がります。",
  壬: "壬（大海の水）が守護神。流れに乗って大胆に動き、視野を広く持つほど運が巡ります。",
  癸: "癸（雨露の水）が守護神。潤いと知恵を行き渡らせ、情報・気配りで調整役に回ると運が開きます。",
};

/* 調候用神表（窮通宝鑑系・第一用神）。CHOKO[日干][月支(漢字)] = [主, 副?]。
   ── 採用根拠：徐樂吾評註『窮通宝鑑』に示される各日干・各月の調候用神のうち、
      最も主とされる干（第一用神）を1つ確定。副は併記が一般的なもののみ。
   ── 土性(戊/己)は窮通宝鑑では火・水・木を調候に用いるため、それに準拠。 */
const CHOKO = {
  // 甲木：春は庚で剪定、夏は癸で潤し、秋は庚丁、冬は丁庚で暖める
  甲: { 寅: ["丙", "癸"], 卯: ["庚", "丙"], 辰: ["庚", "壬"], 巳: ["癸", "庚"], 午: ["癸", "庚"], 未: ["癸", "庚"], 申: ["庚", "丁"], 酉: ["庚", "丁"], 戌: ["庚", "甲"], 亥: ["庚", "丁"], 子: ["丁", "庚"], 丑: ["丁", "庚"] },
  // 乙木：春は丙癸、夏は癸で潤す、秋は癸丙、冬は丙で暖める
  乙: { 寅: ["丙", "癸"], 卯: ["丙", "癸"], 辰: ["癸", "丙"], 巳: ["癸", "丙"], 午: ["癸", "丙"], 未: ["癸", "丙"], 申: ["丙", "癸"], 酉: ["癸", "丙"], 戌: ["癸", "辛"], 亥: ["丙", "戊"], 子: ["丙", "癸"], 丑: ["丙", "癸"] },
  // 丙火：春夏は壬で抑え、秋冬は甲壬で生扶
  丙: { 寅: ["壬", "庚"], 卯: ["壬", "己"], 辰: ["壬", "甲"], 巳: ["壬", "庚"], 午: ["壬", "庚"], 未: ["壬", "庚"], 申: ["壬", "甲"], 酉: ["壬", "癸"], 戌: ["甲", "壬"], 亥: ["甲", "壬"], 子: ["壬", "甲"], 丑: ["壬", "甲"] },
  // 丁火：春は甲で生じ、夏は壬庚、秋冬は甲庚で扶ける
  丁: { 寅: ["甲", "庚"], 卯: ["庚", "甲"], 辰: ["甲", "庚"], 巳: ["甲", "庚"], 午: ["壬", "庚"], 未: ["甲", "壬"], 申: ["甲", "庚"], 酉: ["甲", "庚"], 戌: ["甲", "庚"], 亥: ["甲", "庚"], 子: ["甲", "庚"], 丑: ["甲", "庚"] },
  // 戊土：春は丙甲、夏は甲丙癸で潤し、秋は丙癸、冬は丙甲で暖める
  戊: { 寅: ["丙", "甲"], 卯: ["丙", "甲"], 辰: ["甲", "丙"], 巳: ["甲", "丙"], 午: ["壬", "甲"], 未: ["癸", "丙"], 申: ["丙", "癸"], 酉: ["丙", "癸"], 戌: ["甲", "丙"], 亥: ["甲", "丙"], 子: ["丙", "甲"], 丑: ["丙", "甲"] },
  // 己土：丙で暖め癸で潤すのが基本。夏は癸丙、冬は丙で暖める
  己: { 寅: ["丙", "甲"], 卯: ["甲", "癸"], 辰: ["丙", "癸"], 巳: ["癸", "丙"], 午: ["癸", "丙"], 未: ["癸", "丙"], 申: ["丙", "癸"], 酉: ["丙", "癸"], 戌: ["甲", "丙"], 亥: ["丙", "甲"], 子: ["丙", "甲"], 丑: ["丙", "甲"] },
  // 庚金：春は丁甲で鍛え、夏は壬で清め、秋は丁甲、冬は丁丙で暖める
  庚: { 寅: ["丙", "甲"], 卯: ["丁", "甲"], 辰: ["甲", "丁"], 巳: ["壬", "戊"], 午: ["壬", "癸"], 未: ["丁", "甲"], 申: ["丁", "甲"], 酉: ["丁", "甲"], 戌: ["甲", "壬"], 亥: ["丁", "丙"], 子: ["丁", "丙"], 丑: ["丙", "丁"] },
  // 辛金：壬で洗い清めるのが第一。冬は丙で暖める
  辛: { 寅: ["己", "壬"], 卯: ["壬", "甲"], 辰: ["壬", "甲"], 巳: ["壬", "甲"], 午: ["壬", "己"], 未: ["壬", "庚"], 申: ["壬", "甲"], 酉: ["壬", "甲"], 戌: ["壬", "甲"], 亥: ["壬", "丙"], 子: ["丙", "壬"], 丑: ["丙", "壬"] },
  // 壬水：春は庚で水源、夏は癸庚で冷やし、秋は戊で堤、冬は丙戊で暖め抑える
  壬: { 寅: ["庚", "丙"], 卯: ["戊", "辛"], 辰: ["甲", "庚"], 巳: ["壬", "庚"], 午: ["癸", "庚"], 未: ["辛", "甲"], 申: ["戊", "丁"], 酉: ["甲", "庚"], 戌: ["甲", "丙"], 亥: ["戊", "丙"], 子: ["戊", "丙"], 丑: ["丙", "丁"] },
  // 癸水：春は辛で水源、夏は庚辛で生じ、秋は辛丙、冬は丙で暖める
  癸: { 寅: ["辛", "丙"], 卯: ["庚", "辛"], 辰: ["丙", "辛"], 巳: ["辛", "庚"], 午: ["庚", "辛"], 未: ["庚", "辛"], 申: ["丁", "甲"], 酉: ["辛", "丙"], 戌: ["辛", "甲"], 亥: ["庚", "辛"], 子: ["丙", "辛"], 丑: ["丙", "丁"] },
};

/* 守護神（調候用神）を生年月日（出生情報）から算出。
   ◆ 入力 birth: computeChart と同じ { date, time?, utcOffset?, lat?, lon? }
   ◆ 出力:
       {
         stem,       // 守護神の十干（例 "壬"）
         element,    // その五行（例 "水"）
         label,      // 例「壬水」
         sub,        // 副用神の十干（無ければ null）
         subLabel,   // 例「庚金」（無ければ null）
         meaning,    // 守護神の意味・活かし方（1〜2文）
         color,      // 五行→開運色（例「黒・青」）
         direction,  // 五行→方位（例「北」）
         dayMaster,  // 日干（参考）
         monthBranch,// 月支（節月。参考）
         summary,    // やさしい総括1文
       }
   ◆ birth 不正・空は try/catch で null（落ちない）。 */
export function shugojin(birth) {
  try {
    if (!birth || !birth.date) return null;
    const c = computeChart(birth);
    const dayMaster = c.dayMaster;                       // 日干
    const monthBranch = String(c.monthPillar).slice(-1); // 月支（節月）
    if (!dayMaster || !monthBranch) return null;

    const row = CHOKO[dayMaster];
    if (!row) return null;
    const entry = row[monthBranch];
    if (!entry || !entry.length) return null;

    const stem = entry[0];
    const sub = entry[1] || null;
    const element = FIVE[stem];
    if (!element) return null;

    const color = ELEM_COLOR[element] || "";
    const direction = ELEM_DIR[element] || "";
    const meaning = SHUGO_MEANING[stem] || `${STEM_ELEM_LABEL(stem)}が守護神。この五行を補うと運が整います。`;

    const summary = `あなたの守護神は${STEM_ELEM_LABEL(stem)}。開運色は${color}、ラッキー方位は${direction}です。`;

    return {
      stem,
      element,
      label: STEM_ELEM_LABEL(stem),
      sub,
      subLabel: sub ? STEM_ELEM_LABEL(sub) : null,
      meaning,
      color,
      direction,
      dayMaster,
