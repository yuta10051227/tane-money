...
- **売上（タブ）** — 定期収入（顧問料・月額講座など）を登録すれば毎月自動計上。単発売上の記録、12ヶ月推移、そして固定費と連動した **今月の手残り＝売上 − 固定費** を表示。端末内の自動分析「秘書の気づき」（離脱検知・売上予測・トレンド）付き
- **AI秘書（秘書タブ）** — 「今月どう？」と話しかけると、あなたの売上・固定費の要約をもとに Claude が相談に乗る会話型秘書（要セットアップ・下記）
...
「秘書」タブの会話型AIは、Vercelのサーバレス関数 `api/chat.js` 経由で Claude を呼びます。APIキーはサーバ側だけに置き、ブラウザには出しません。

1. <https://console.anthropic.com> で APIキーを発行
2. Vercel → プロジェクト → Settings → Environment Variables に **`ANTHROPIC_API_KEY`** を登録（再デプロイで有効化）
3. （任意）`CLAUDE_MODEL` を `claude-sonnet-4-6` や `claude-haiku-4-5` にするとコストを抑えられます（既定は `claude-opus-4-8`）

> 未設定でもアプリは普通に動きます（秘書タブだけ「未設定」と表示）。相談内容と財務サマリーは回答生成のため Anthropic に送信されます。
